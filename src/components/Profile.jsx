...
import {
  User, Clock, BarChart2, Briefcase, CheckCircle,
  Globe, Trophy, RefreshCw, TrendingUp, Zap, Calendar,
  Film, Activity, ChevronRight, Layers, Key, Settings, Eye, EyeOff, Check,
} from "lucide-react";
...
const PIE_COLORS = ["#12a0e1", "#1cc1a5", "#8b5cf6", "#f59e0b", "#ec4899", "#10b981"];
  ...
  return (
    <div className="bg-white border border-[#dce4ec] rounded-2xl p-4 shadow-sm flex flex-col gap-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-black text-[#768994] uppercase tracking-widest">{label}</span>
      </div>
...
function WrikeTaskCard({ task, filter }) {
  ...
  return (
    <div className={`p-4 border-y border-r border-l-4 rounded-2xl ${borderColor} ${
      isMatrix
      ...
      <p className={`text-sm font-bold ${isMatrix ? "text-slate-400" : "text-[#122027]"}`}>
        {task.title}
      </p>
    </div>
  );
...
function JobsSection({ wrikeUser, filter }) {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const [aJson, cJson, wJson] = await Promise.all([aRes.json(), cRes.json(), wRes.json()]);
      ...
      const isChild = (t) => (t.superTaskIds || []).some((pid) => assignedIds.has(pid));

      const enrich = (t) => {
        const customStatusName = t.customStatusId
          ? (statusNameMap[t.customStatusId] || t.status) : t.status;
        ...
        let projectName = Object.keys(filmFreq).length
          ...
          : "";
        if (!projectName) {
          const m = t.title.match(/^(?:Edit|Design|Animation|Motion|Finish|Grade|Sound|Audio|VFX|Colourist|DI)\s*[-–]\s*(.+?)(?:\s*[-–]\s*.+)?$/i);
          if (m) projectName = m[1].trim();
        }
        if (!projectName) projectName = t.title.split(/[_\-]/)[0].trim() || "Other Projects";
        return { ...t, customStatusName, isDone, projectName };
      };

      const active = (aJson.data || []).filter((t) => !isChild(t)).map(enrich);
      const completed = (cJson.data || []).filter((t) => !isChild(t)).map(enrich).filter((t) => t.isDone);
  ...
  const sortedCampaigns = useMemo(() => Object.keys(grouped).sort((a, b) => {
    const latest = (ts) => Math.max(...ts.map((t) =>
      new Date(filter === "completed" ? (t.completedDate || t.updatedDate || 0) : (t.updatedDate || 0)).getTime()
    ));
  ...
  if (fetched && sorted.length === 0) return (
    <Empty icon={Icon} message={`No ${filter === "completed" ? "completed" : "active"} jobs found.`} />
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={fetchTasks} className="flex items-center gap-1.5 text-[11px] font-bold text-[#768994] hover:text-[#12a0e1] transition-colors">
        ...
        <div key={campaign}>
          <div className="flex items-center gap-2 text-xs font-black text-[#12a0e1] uppercase tracking-widest mb-3 border-b border-[#dce4ec] pb-2">
            ...
            <span className="ml-auto bg-slate-100 text-[#768994] px-2 py-0.5 rounded text-[10px] normal-case tracking-normal font-bold">
              {grouped[campaign].length} {grouped[campaign].length === 1 ? "job" : "jobs"}
            </span>
          </div>
...
function OverviewSection({ tasks, wrikeUser, userStats, handleFetchLifetimeStats }) {
  const totalSeconds = tasks.reduce((s, t) => s + (t.rawSeconds ?? 0) + (t.additionalSeconds ?? 0), 0);
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`https://www.wrike.com/api/v4/contacts/${uid}/timelogs?plainText=true`, { headers }),
      fetch("https://www.wrike.com/api/v4/timers", { headers }),
    ])
      .then(([lRes, tRes]) => Promise.all([lRes.json(), tRes.json()]))
      .then(async ([lJson, tJson]) => {
        const loggedIds = new Set(tasks.map((t) => t.wrikeTimelogId).filter(Boolean));
        ...
        for (let i = 0; i < taskIds.length; i += BATCH) {
          ...
          try {
            ...
            if (res.ok) {
              ...
              (json.data || []).forEach((t) => { taskTitles[t.id] = t.title; });
            }
          } catch (e) { /* silent */ }
        }
        ...
        const logs = recentLogs.map((l) => ({
          ...
          sub: l.comment && l.comment !== taskTitles[l.taskId] ? l.comment : null,
          duration: l.hours != null ? formatDurationText(Math.round(l.hours * 3600)) : null,
  ...
  const loggedTimelogIds = new Set(tasks.map((t) => t.wrikeTimelogId).filter(Boolean));
  const localFallback = [...tasks]
    ...
    .map((t) => ({
      ...
      duration: formatDurationText((t.rawSeconds ?? 0) + (t.additionalSeconds ?? 0)),
      date: t.date || null,
  ...
  const territories = useMemo(() => {
    const counts = {};
    tasks.forEach((t) => { if (t.territory) counts[t.territory] = (counts[t.territory] || 0) + 1; });
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Tasks logged"      value={tasks.length}                                unit="rows"  icon={Clock}       accent="#12a0e1" />
        <StatCard label="Total time"        value={formatDurationText(totalSeconds)}            unit=""      icon={TrendingUp}  accent="#1cc1a5" />
        <StatCard label="30-day deliveries" value={userStats.fetched ? userStats.month : "—"}  unit="done"  icon={CheckCircle} accent="#8b5cf6" />
        <StatCard label="All-time"          value={userStats.fetched ? userStats.allTime : "—"} unit="tasks" icon={Trophy}      accent="#f59e0b" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Activity feed */}
        <div className="bg-white border border-[#dce4ec] rounded-2xl p-5 shadow-sm">
          <SectionTitle icon={Zap} title="Recent activity"
          ...
          {activityFeed.length === 0
            ...
            : activityFeed.slice(0, 8).map((a) => (
              <div key={a.id} className="flex gap-3 items-start py-2.5 border-b border-[#f1f5f9] last:border-0">
                {/* Dot — pulsing for live timer, amber for pending, blue for logged */}
                <div className={`mt-2 shrink-0 ${a.live ? "relative" : ""}`}>
                  <div className={`w-2 h-2 rounded-full ${
                    a.live    ? "bg-amber-400" :
                    a.pending ? "bg-amber-400" :
                                "bg-[#12a0e1]"
                  }`} />
                  {a.live && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-bold text-[#122027] truncate">{a.label}</p>
                    {a.live && (
                ...
                {a.duration && !a.live && (
                  <span className="text-sm font-black text-[#12a0e1] shrink-0">{a.duration}</span>
                )}
              </div>
            ))
          }
            ...
            : (
              <div className="flex flex-wrap gap-2">
                {territories.map(([terr, count]) => (
                  <span key={terr} className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border border-[#dce4ec] bg-slate-50 text-[#122027]">
                    {TERRITORY_FLAGS[terr] || "🌍"} {terr}
                    <span className="text-[10px] font-black text-[#12a0e1] bg-[#12a0e1]/10 px-1.5 py-0.5 rounded-full">{count}</span>
                  </span>
                ))}
              </div>
            )
      ...
      {wrikeUser && !userStats.fetched && (
        <div className="bg-[#12a0e1]/5 border border-[#12a0e1]/20 rounded-2xl p-4 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-black text-[#122027]">Sync your lifetime Wrike stats</p>
            <p className="text-xs text-[#768994] mt-0.5">Fetch all-time completed task count from Wrike</p>
          </div>
          <button
          ...
          >
            <RefreshCw className={`w-3.5 h-3.5 ${userStats.loading ? "animate-spin" : ""}`} />
...
const DAY_COLORS = {
  Monday:    { bg: "bg-violet-50",  border: "border-l-violet-400",  text: "text-violet-600",  pill: "bg-violet-100 text-violet-700" },
  Tuesday:   { bg: "bg-sky-50",     border: "border-l-sky-400",     text: "text-sky-600",     pill: "bg-sky-100 text-sky-700" },
  Wednesday: { bg: "bg-teal-50",    border: "border-l-teal-400",    text: "text-teal-600",    pill: "bg-teal-100 text-teal-700" },
  Thursday:  { bg: "bg-amber-50",   border: "border-l-amber-400",   text: "text-amber-600",   pill: "bg-amber-100 text-amber-700" },
  Friday:    { bg: "bg-rose-50",    border: "border-l-rose-400",    text: "text-rose-600",    pill: "bg-rose-100 text-rose-700" },
};
const DEFAULT_DAY = { bg: "bg-slate-50", border: "border-l-slate-300", text: "text-slate-500", pill: "bg-slate-100 text-slate-600" };
...
function HistorySection({ tasks }) {
  ...
  const filtered = useMemo(() =>
    (dayFilter === "all" ? tasks : tasks.filter((t) => t.dayOfWeek === dayFilter))
      .sort((a, b) => b.id - a.id)
  , [tasks, dayFilter]);
  ...
  const grouped = useMemo(() => {
    ...
    const order = [...DAY_ORDER.filter((d) => map[d]).reverse(), ...Object.keys(map).filter((d) => !DAY_ORDER.includes(d))];
    return { map, order };
  }, [filtered]);

  return (
    <div className="space-y-4">
      <SectionTitle icon={Clock} title="Timesheet history"
        right={
              ...
              return (
                <button key={d} onClick={() => setDayFilter(d)}
                  className={`text-[10px] font-black px-2.5 py-1 rounded-lg transition-all uppercase tracking-wider ${
                    isActive
                      ? dc ? `bg-white shadow-sm ${dc.text}` : "bg-white text-[#12a0e1] shadow-sm"
              ...
              return (
                <div key={dayKey}>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className={`flex items-center gap-2 text-xs font-black uppercase tracking-widest mb-3 border-b border-[#dce4ec] pb-2 ${dc.text}`}>
                        <span className={`px-2 py-0.5 rounded-md text-[10px] ${dc.pill}`}>{dayKey}</span>
                        <span className="ml-auto bg-slate-100 text-[#768994] px-2 py-0.5 rounded text-[10px] normal-case tracking-normal font-bold shrink-0">
                          {formatDurationText(groupTotal)} · {rows.length} {rows.length === 1 ? "row" : "rows"}
                        </span>
                      </div>
                    );
                  })()}
                    ...
                    {rows.map((t) => {
                      ...
                      >
                        ...
                        <div className="shrink-0 text-center w-14">
                          {t.date && <p className="text-[11px] font-bold text-[#768994]">{t.date}</p>}
                        </div>

                        {/* Meta */}
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-black text-[#122027] truncate">{t.jobNumber || "Unknown job"}</p>
                          <div className="flex items-center gap-2 flex-wrap mt-0.5">
                            {t.territory && (
                              <span className="text-[11px] font-bold text-[#768994]">
                                {TERRITORY_FLAGS[t.territory] || "🌍"} {t.territory}
                              </span>
                            )}
                            {t.category && (
                          ...
                          {t.notes && (
                            <p className="text-[11px] text-[#768994] mt-1 italic truncate">{t.notes}</p>
                          )}
                        </div>

                        {/* Time */}
                        <div className="text-right shrink-0">
                          <p className="text-sm font-black text-[#12a0e1]">{formatDurationText(totalSecs(t))}</p>
                          {(t.additionalSeconds ?? 0) > 0 && (
                            <p className="text-[10px] text-[#1cc1a5] font-bold">+{formatDurationText(t.additionalSeconds)}</p>
                          )}
                        </div>
                      </div>
                    );
...
function AnalyticsSection({ tasks }) {
  const tooltipStyle = {
    backgroundColor: "#ffffff", borderColor: "#dce4ec", borderRadius: "12px",
    color: "#323b43", fontSize: "11px", fontWeight: 600,
  ...
  const timePerJob = useMemo(() => {
    ...
    tasks.forEach((t) => {
      ...
      map[key].mins += Math.round(((t.rawSeconds ?? 0) + (t.additionalSeconds ?? 0)) / 60);
    });
    return Object.values(map).sort((a, b) => b.mins - a.mins).slice(0, 8);
  }, [tasks]);
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-white border border-[#dce4ec] rounded-2xl p-5 shadow-sm">
          ...
          {timePerJob.length === 0 ? <Empty icon={BarChart2} message="No data yet." /> : (
            <div style={{ height: 230 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={timePerJob} margin={{ top: 4, right: 4, left: -28, bottom: 30 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="name" axisLine={false} tickLine={false}
                    tick={{ fill: "#768994", fontSize: 9, fontWeight: 600 }}
                    angle={-30} textAnchor="end" dy={10} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: "#768994", fontSize: 10, fontWeight: 600 }} />
                  <Tooltip contentStyle={tooltipStyle} formatter={(v) => [`${v} mins`, "Time"]} />
        ...
        <div className="bg-white border border-[#dce4ec] rounded-2xl p-5 shadow-sm">
          ...
          {byTerritory.length === 0 ? <Empty icon={Globe} message="No data yet." /> : (
            <div style={{ height: 230 }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={byTerritory} cx="45%" cy="50%" innerRadius={50} outerRadius={85}
                    paddingAngle={3} dataKey="value" stroke="none">
                    {byTerritory.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                  </Pie>
                  <Tooltip contentStyle={tooltipStyle} formatter={(v) => [`${v} mins`, "Time"]} />
                  <Legend iconType="circle" layout="vertical" verticalAlign="middle" align="right"
                    wrapperStyle={{ fontSize: "10px", fontWeight: 600, color: "#768994" }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {byCategory.map(({ name, value }) => {
            ...
            return (
              <div key={name} className="flex items-center gap-3">
                <span className="text-[11px] font-bold text-[#768994] w-36 shrink-0 truncate">{name}</span>
                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-[#12a0e1] rounded-full" style={{ width: `${Math.round((value / max) * 100)}%` }} />
                </div>
...
function SettingsSection() {
  const [token, setToken] = useState(() => localStorage.getItem("wrike_personal_token") || "");
    ...
    <div className="space-y-6">
      ...
      <div className="border border-[#dce4ec] rounded-2xl overflow-hidden">
        <div className="bg-slate-50 border-b border-[#dce4ec] px-5 py-3 flex items-center gap-2">
          ...
          <span className="text-sm font-black text-[#122027]">Wrike Personal Token</span>
          {hasToken && (
            <span className="ml-auto text-[10px] font-black text-[#1cc1a5] bg-[#1cc1a5]/10 px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full bg-[#1cc1a5]" /> Connected
            </span>
          )}
          {!hasToken && (
        ...
        <div className="p-5 space-y-4">
          <p className="text-xs text-[#768994] leading-relaxed">
            Your Wrike permanent token is used to fetch tasks, timelogs, and timers. It's stored locally in your browser and never sent to our servers.
            You can generate one from <span className="font-bold text-[#12a0e1]">Wrike → Profile → Apps & Integrations → API</span>.
          </p>
      ...
      <div className="bg-slate-50 border border-[#dce4ec] rounded-2xl p-5">
        <p className="text-xs font-black text-[#768994] uppercase tracking-widest mb-3">What the token unlocks</p>
        <div className="space-y-2">
          {[
            ...
            <div key={item} className="flex items-center gap-2 text-sm text-[#768994]">
...
export default function Profile({ wrikeData }) {
  ...
  useEffect(() => {
    ...
    if (!uid) return;
    supabase.from("profiles").select("*").eq("wrike_user_id", uid).single()
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 pt-8 space-y-6">
        ...
        <div className="bg-white border border-[#dce4ec] rounded-[2rem] overflow-hidden shadow-sm">
          ...
          <div className="p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            {/* Identity */}
            <div className="flex-1 min-w-0">
              <h1 className="text-3xl font-black tracking-tight text-[#122027]">{displayName}</h1>
              <div className="flex flex-wrap items-center gap-2.5 mt-2">
                {profile?.email && (
                  <span className="text-xs text-[#768994] font-medium">{profile.email}</span>
                )}
                {wrikeUser?.id && (
                  <span className="text-[10px] font-black text-[#768994] bg-slate-100 px-2.5 py-1 rounded-full uppercase tracking-wider">
                ...
                {profile?.updated_at && (
                  <span className="text-[10px] font-black text-[#1cc1a5] bg-[#1cc1a5]/10 px-2.5 py-1 rounded-full uppercase tracking-wider flex items-center gap-1">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#1cc1a5]" /> Active
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-4 shrink-0">
              {[
                ...
                <div key={label} className="text-center">
                  <div className="text-xl font-black text-[#122027]">{value}</div>
        ...
        {!hasToken && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-black text-amber-800">Wrike token not set</p>
              <p className="text-xs text-amber-600 mt-0.5">Add your personal token to unlock timelogs, live timers, and job syncing.</p>
            </div>
            <button
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <div className="bg-white border border-[#dce4ec] rounded-2xl p-6 shadow-sm">
                {activeSection === "overview"  && <OverviewSection tasks={tasks} wrikeUser={wrikeUser} userStats={userStats} handleFetchLifetimeStats={handleFetchLifetimeStats} />}
                {activeSection === "history"   && <HistorySection tasks={tasks} />}
                {activeSection === "analytics" && <AnalyticsSection tasks={tasks} />}
                {activeSection === "jobs"      && <JobsSection wrikeUser={wrikeUser} filter="active" />}
                {activeSection === "completed" && <JobsSection wrikeUser={wrikeUser} filter="completed" />}
                {activeSection === "settings"  && <SettingsSection onSave={() => setHasToken(true)} />}
              </div>
            )}
          </div>
        </div>
