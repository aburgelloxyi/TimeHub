...
function HistorySection({ tasks }) {
  ...
  // Group by job number, preserving newest-first order within each group
  const grouped = useMemo(() => {
    const map = {};
    const order = [];
    filtered.forEach((t) => {
      const key = t.jobNumber || "Unknown job";
      if (!map[key]) {
        map[key] = [];
        order.push(key);
      }
    ...
    return { map, order: [...new Set(order)] };
  }, [filtered]);
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-5">
          {grouped.order.map((jobKey) => {
            const rows = grouped.map[jobKey];
            const groupTotal = rows.reduce((s, t) => s + totalSecs(t), 0);
            return (
              <div key={jobKey}>
                {/* Campaign-style group header */}
                <div className="flex items-center gap-2 text-xs font-black text-[#12a0e1] uppercase tracking-widest mb-3 border-b border-[#dce4ec] pb-2">
                  <Layers className="w-3.5 h-3.5" />
                  <span className="truncate flex-1">{jobKey}</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="shrink-0 text-center w-14">
                          <span
                            className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md ${dc.pill}`}
                          >
                            {t.dayOfWeek?.slice(0, 3) || "—"}
                          </span>
                          {t.date && (
                            <p className="text-[11px] font-bold text-[#768994] mt-1">
                              {t.date}
                            </p>
                          )}
                        </div>
